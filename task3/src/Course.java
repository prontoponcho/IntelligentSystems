...
public class Course {
	...
	public static Coordinate randCoord() {
	...
	public static void rotate(ArrayList<Coordinate> coords, double yaw, double rotation) {
		...
		double deltaZ = 0.5 * Math.cos(Math.toRadians(newYaw));
		double deltaX = 0.5 * Math.sin(Math.toRadians(newYaw));
		coords.add(new Coordinate(deltaX + c1.getX(), c1.getY(), deltaZ + c1.getZ()));
	}
	
//	public static ArrayList<Coordinate> rotate(Coordinate start, double yaw, double rotation) {
//		double newYaw = (yaw + rotation) % 360;
//		double deltaZ = 1 * Math.cos(Math.toRadians(newYaw));
//		double deltaX = 1 * Math.sin(Math.toRadians(newYaw));
//		ArrayList<Coordinate> newCourse = new ArrayList<Coordinate>();
//		newCourse.add(start);
//		newCourse.add(new Coordinate(deltaX + start.getX(), start.getY(), deltaZ + start.getZ()));
//		return newCourse;
//	}
	
	public static ArrayList<Coordinate> rotateByDegrees(Coordinate start, double yaw, double degree) {
		double deltaZ = 1 * Math.cos(Math.toRadians(yaw + degree));
		double deltaX = 1 * Math.sin(Math.toRadians(yaw + degree));
		ArrayList<Coordinate> newCourse = new ArrayList<Coordinate>();
		newCourse.add(start);
		newCourse.add(new Coordinate(deltaX + start.getX(), start.getY(), deltaZ + start.getZ()));
		return newCourse;
	...
	public static ArrayList<Coordinate> verticalArc(double radius, double frequency, Coordinate start, double yaw) {
		System.out.println("start coordinate: " + start.toString());
		ArrayList<Coordinate> coords = new ArrayList<Coordinate>();
		coords.add(start);
		for (int angle = 5; angle < 180; angle++) {
			Coordinate curr = coords.get(coords.size() - 1);
			double deltaZ = 1 * Math.cos(Math.toRadians(yaw));
			double deltaX = 1 * Math.sin(Math.toRadians(yaw));
			double y = radius * Math.sin(Math.toRadians(frequency*angle));
			if (y < 3) {
				coords.add(new Coordinate(deltaX + curr.getX(), y, deltaZ + curr.getZ()));
				extendLength(coords, 10);
				break;
			}
			if (inBounds(curr, deltaX, deltaZ))
				coords.add(new Coordinate(deltaX + curr.getX(), y, deltaZ + curr.getZ()));
			else {
				rotate(coords, yaw, 180);
				extendLength(coords, 10);
				//break;
			}
		}
